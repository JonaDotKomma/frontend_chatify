...
function Enviomasivo() {
    ...
    useEffect(() => {
        ...
        axios.get('/getcate') // Asegúrate de que la URL sea correcta
            .then((response) => {
    ...
    const listChatByCategory = async (idUser, category) => {
        if (category) {
            try {
                ...
                const response = await axios.post('/listuserbycategory', { idUser, idCategoria: parseInt(category) });
                const jsonData = response.data;
    ...
    const sendTemplates = async (allChats) => {
        ...
        for (const chat of allChats) {
            ...
            try {
                const response = await axios.post('/initChat', postData);
